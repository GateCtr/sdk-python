...
async function checkWebhookSetup() {
  ...
  try {
    ...
    if (roles.length === 0) {
      ...
      if (developerRole) {
        ...
        console.log(`   → ${developerRole.rolePermissions.length} permissions assignées`);
      } else {
  ...
  try {
    const tables = [
      ...
      { name: 'permissions', check: async () => await prisma.permission.findFirst() },
      { name: 'user_roles', check: async () => await prisma.userRole.findFirst() },
      { name: 'audit_logs', check: async () => await prisma.auditLog.findFirst() },
      { name: 'email_logs', check: async () => await prisma.emailLog.findFirst() },
    ];
  ...
  if (hasErrors) {
    ...
    console.log('   4. Démarrez le tunnel: clerk listen --forward-to http://localhost:3000/api/webhooks/clerk');
