...
import { ArrowRight, Shield, Zap, GitBranch, BarChart3, Webhook, Users } from 'lucide-react';
...
// ─── Deep-dive tab ids ────────────────────────────────────────────────────────
const DEEP_TABS = ['budget', 'optimizer', 'router', 'analytics', 'webhooks'] as const;
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {gridItems.map((item, i) => {
              ...
              const planColor = PLAN_COLORS[item.plan] ?? 'bg-muted text-muted-foreground';
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className={`inline-block mt-0.5 rounded-full px-2 py-0.5 text-[10px] font-medium ${planColor}`}>
                          {item.plan}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="text-right shrink-0">
                      <div className="text-xl font-bold text-foreground tabular-nums">{item.metric}</div>
                      <div className="text-[10px] text-muted-foreground">{item.metricLabel}</div>
                    </div>
                  </div>

                  {/* Tagline */}
                  <p className="text-sm font-medium text-foreground/80">{item.tagline}</p>
      ...
      <section className="py-20 px-4 bg-muted/20">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            {/* Left — copy */}
            <div>
              <h3 className="text-2xl font-bold text-foreground mb-3">{deepData.headline}</h3>
              <p className="text-muted-foreground leading-relaxed mb-6">{deepData.description}</p>
