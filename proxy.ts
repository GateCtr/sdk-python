...
export default clerkMiddleware(
  ...
  if (geoEnabled && !isBlockedPage && !pathname.startsWith('/api')) {
    ...
    if (country && !ALLOWED_COUNTRIES.includes(country)) {
      const blockedPath = pathname.startsWith('/fr') ? '/fr/blocked' : '/blocked';
  ...
  if (userId && !isPublicRoute(req)) {
    ...
    let onboardingDone = sessionClaims?.publicMetadata?.onboardingComplete === true;

    if (!onboardingDone) {
      try {
        const client = await clerkClient();
        const user = await client.users.getUser(userId);
        onboardingDone = (user.publicMetadata as { onboardingComplete?: boolean })?.onboardingComplete === true;
      } catch {
