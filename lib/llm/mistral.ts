...
async function handleProviderError(res: Response): Promise<never> {
  ...
  throw new ProviderError("mistral", res.status, message, res.status >= 500);
}

// Mistral uses /chat/completions for all models — route complete() through chat().
...
export async function chat(
  ...
  try {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      throw new ProviderError("mistral", 408, "Request timeout", true);
    }
    throw err;
  } finally {
    clearTimeout(timer);
