...
function useBreadcrumb() {
  ...
  const segments = Object.entries(ROUTE_LABELS)
    .filter(([route]) => clean === route || (route !== '/dashboard' && clean.startsWith(route)))
...
function TokenUsagePill({ plan }: { plan: string }) {
  ...
  const limitLabel = upper === 'ENTERPRISE'
    ? t('unlimited')
    : limit >= 1_000_000
      ? `${limit / 1_000_000}M`
      : `${limit / 1_000}K`;
      ...
      <div className="flex flex-col gap-1 min-w-[80px]">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="text-[11px] font-semibold tabular-nums">
            {used.toLocaleString()} <span className="text-muted-foreground font-normal">/ {limitLabel}</span>
          </span>
        </div>
        {/* Progress bar */}
  ...
  return (
    <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border bg-background/95 backdrop-blur-sm px-4 sticky top-0 z-10">
      ...
      <div className="flex items-center gap-1.5 text-sm min-w-0">
        <span className="text-muted-foreground/60 hidden sm:inline">GateCtr</span>
        {breadcrumb && (
          <>
