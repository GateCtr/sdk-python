...
export type CodeLanguage = 'ts' | 'tsx' | 'js' | 'jsx' | 'python' | 'bash' | 'json' | 'text';
...
const JS_KEYWORDS = new Set([
  'const','let','var','import','from','export','default','async','await',
  'return','function','class','new','if','else','for','while','do','switch',
  'case','break','continue','try','catch','finally','throw','typeof','instanceof',
  'in','of','void','delete','yield','static','extends','super','this',
  'true','false','null','undefined','NaN','Infinity',
]);

const JS_BUILTINS = new Set([
  'console','process','require','module','exports','Promise','Array','Object',
  'String','Number','Boolean','Error','JSON','Math','Date','RegExp','Map','Set',
  'setTimeout','setInterval','clearTimeout','clearInterval','fetch','Response',
  'Request','Headers','URL','URLSearchParams','Buffer','ReadableStream',
]);

const PY_KEYWORDS = new Set([
  'def','class','import','from','as','return','if','elif','else','for','while',
  'try','except','finally','raise','with','pass','break','continue','lambda',
...
const PY_BUILTINS = new Set([
  ...
  'bool','open','input','super','self','cls','enumerate','zip','map','filter',
...
function tokenizeLine(line: string, lang: CodeLanguage): Token[] {
  ...
  if (line.startsWith('- ') || line === '-') return [{ type: 'deleted', value: line }];
  if (line.startsWith('+ ') || line === '+') return [{ type: 'added', value: line }];
  ...
  while (rest.length > 0) {
    ...
    if (numMatch && (tokens.length === 0 || tokens[tokens.length - 1].type !== 'plain' || /[\s(,=:[\{]$/.test(tokens[tokens.length - 1].value))) {
    ...
    if (identMatch) {
      ...
      else if (tokens.length > 0 && tokens[tokens.length - 1].value === '.') type = 'property';
...
function tokenize(code: string, lang: CodeLanguage = 'ts'): Token[][] {
  ...
  if (lang === 'json') {
    return code.split('\n').map((line) => {
      ...
      while (rest.length > 0) {
        const strMatch = rest.match(/^("(?:[^"\\]|\\.)*")/);
        if (strMatch) { tokens.push({ type: 'string', value: strMatch[1] }); rest = rest.slice(strMatch[1].length); continue; }
        const numMatch = rest.match(/^-?\d+(\.\d+)?/);
        if (numMatch) { tokens.push({ type: 'number', value: numMatch[0] }); rest = rest.slice(numMatch[0].length); continue; }
        const kwMatch = rest.match(/^(true|false|null)/);
        if (kwMatch) { tokens.push({ type: 'keyword', value: kwMatch[1] }); rest = rest.slice(kwMatch[1].length); continue; }
        tokens.push({ type: 'plain', value: rest[0] }); rest = rest.slice(1);
      }
      return tokens.length > 0 ? tokens : [{ type: 'plain', value: line }];
    });
  }
  ...
  if (lang === 'bash') {
    return code.split('\n').map((line) => {
      ...
      while (rest.length > 0) {
        ...
        if (flagMatch) { tokens.push({ type: 'operator', value: flagMatch[1] }); rest = rest.slice(flagMatch[1].length); continue; }
        const cmdMatch = rest.match(/^([a-zA-Z_][a-zA-Z0-9_.-]*)/);
        if (cmdMatch) {
          const isBashCmd = ['npm','npx','pnpm','yarn','node','python','pip','git','curl','export','echo','cd','ls','mkdir','rm','cp','mv','cat','grep','sed','awk','chmod','sudo','docker','kubectl'].includes(cmdMatch[1]);
          tokens.push({ type: isBashCmd ? 'builtin' : 'plain', value: cmdMatch[1] });
...
const LANG_LABEL: Record<CodeLanguage, string> = {
  ...
  python: 'Python', bash: 'Bash', json: 'JSON', text: '',
};
    ...
    <div className={cn(
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border/40 bg-grey-800/60 gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          {/* Filename */}
          {filename && (
            <span className="text-xs font-mono text-grey-400 truncate shrink-0">{filename}</span>
          )}
          ...
          {tabs.length === 1 && tabs[0].label && !filename && (
            <span className="text-xs font-mono text-grey-400 truncate">{tabs[0].label}</span>
          )}
        </div>
          ...
          {allowWrap && (
            <button
              ...
              className={cn(
                ...
                wrapped ? 'text-primary-300' : 'text-grey-500 hover:text-grey-200'
              )}
            >
      ...
      >
        ...
        <pre
          className={cn(
            ...
            wrapped ? 'whitespace-pre-wrap break-all' : 'overflow-x-auto whitespace-pre'
          )}
        >
          <code>
            {lines.map((lineTokens, li) => {
              ...
              return (
                <span
                  ...
                  className={cn(
                    'flex min-w-0',
                    isHighlighted && 'bg-warning-400/8 border-l-2 border-warning-400'
                  )}
                >
                  {/* Line number gutter */}
                  {showLineNumbers && (
                  ...
                  {/* Code content */}
                  <span className={cn('flex-1', showLineNumbers ? 'pr-5' : 'px-5')}>
      ...
      {current.callout && (
        <div className="px-5 pb-4">
          <div className={cn(
            ...
            <span className={cn('size-1.5 rounded-full animate-pulse shrink-0', CALLOUT_DOT[calloutVariant])} />
            {current.callout}
          </div>
        </div>
      )}
