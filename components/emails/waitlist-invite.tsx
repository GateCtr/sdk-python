...
export default function WaitlistInviteEmail({ name, email, inviteCode, expiresAt, expiryDays: expiryDaysProp }: WaitlistInviteEmailProps) {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://gatectr.com';
  const signupUrl = `${appUrl}/sign-up?invite=${inviteCode}`;
  const expiryDays = expiryDaysProp ?? Math.ceil((expiresAt.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={content}>
            ...
            <Text style={text}>
              Connect in 5 min. Your API key stays yours. GateCtr routes, optimizes, and controls — you just save.
            </Text>
            ...
            <Section style={ctaWrap}>
              <Button href={signupUrl} style={button}>Create account</Button>
            </Section>

            <Text style={linkHint}>
              Or: <Link href={signupUrl} style={inlineLink}>{signupUrl}</Link>
            </Text>

            <Section style={steps}>
              <Text style={stepsTitle}>5 min setup</Text>
              <Text style={step}>1. Create your account with {email}</Text>
              <Text style={step}>2. Add your LLM API keys (OpenAI, Anthropic, Mistral&hellip;)</Text>
              <Text style={step}>3. Swap your endpoint URL. That&apos;s it.</Text>
            </Section>

            <Section style={expiry}>
              <Text style={expiryText}>⏱ Expires in {expiryDays} {expiryDays === 1 ? 'day' : 'days'}.</Text>
            </Section>
...
const text = { color: '#4a5568', fontSize: '15px', lineHeight: '1.7', margin: '0 0 28px' };
const inviteBox = {
...
const inviteLabel = { color: 'rgba(255,255,255,0.8)', fontSize: '12px', fontWeight: '600', textTransform: 'uppercase' as const, letterSpacing: '0.05em', margin: '0 0 8px' };
const inviteCode_ = { color: '#ffffff', fontSize: '28px', fontWeight: '700', letterSpacing: '3px', fontFamily: 'monospace', margin: '0' };
...
const linkHint = { color: '#a0aec0', fontSize: '12px', textAlign: 'center' as const, margin: '0 0 32px' };
const inlineLink = { color: '#00B4C8', textDecoration: 'none' };
const steps = { backgroundColor: '#f8fafc', borderRadius: '8px', padding: '20px 24px', margin: '0 0 20px' };
const stepsTitle = { color: '#718096', fontSize: '12px', fontWeight: '600', textTransform: 'uppercase' as const, letterSpacing: '0.05em', margin: '0 0 12px' };
const step = { color: '#4a5568', fontSize: '14px', lineHeight: '1.8', margin: '0 0 4px' };
const expiry = { backgroundColor: '#fffbeb', border: '1px solid #fcd34d', borderRadius: '6px', padding: '12px 16px', margin: '0 0 20px', textAlign: 'center' as const };
const expiryText = { color: '#92400e', fontSize: '13px', fontWeight: '600', margin: '0' };
