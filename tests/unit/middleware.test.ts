...
vi.mock('@clerk/nextjs/server', () => ({
  clerkMiddleware: vi.fn((handler: (auth: unknown, req: unknown) => unknown) => handler),
...
function sanitizeRedirectUrl(raw: string | null, requestUrl: string): string | null {
  if (!raw) return null;
  try {
...
describe('sanitizeRedirectUrl', () => {
  it('returns the path for a same-origin relative URL', () => {
    expect(sanitizeRedirectUrl('/dashboard', 'http://localhost:3000/sign-in')).toBe('/dashboard');
  });

  it('returns path + query for a relative URL with query params', () => {
    expect(
      sanitizeRedirectUrl('/dashboard?tab=billing', 'http://localhost:3000/sign-in'),
    ).toBe('/dashboard?tab=billing');
  });

  it('returns null for an external domain', () => {
    expect(sanitizeRedirectUrl('https://evil.com/steal', 'http://localhost:3000/sign-in')).toBeNull();
  });

  it('returns null for a protocol-relative external URL', () => {
    expect(sanitizeRedirectUrl('//evil.com/path', 'http://localhost:3000/sign-in')).toBeNull();
  });

  it('returns null for null input', () => {
    expect(sanitizeRedirectUrl(null, 'http://localhost:3000/sign-in')).toBeNull();
  });

  it('strips the host from an absolute same-origin URL', () => {
    expect(
      sanitizeRedirectUrl('http://localhost:3000/dashboard', 'http://localhost:3000/sign-in'),
    ).toBe('/dashboard');
  });
});
...
describe('Redirect URL construction', () => {
  ...
  describe('Unauthenticated users are redirected to sign-in (Req 2.1)', () => {
    ...
    it('preserves query parameters in redirect_url', () => {
      ...
      expect(url.pathname).toBe('/sign-in');
      expect(url.searchParams.get('redirect_url')).toBe('/dashboard?tab=billing');
    });
  });
...
describe('Public routes allow unauthenticated access (Req 2.2)', () => {
  ...
  const protectedPaths = ['/dashboard', '/admin/users', '/fr/dashboard', '/profile', '/settings'];
...
describe('Admin routes require admin role (Req 6.2)', () => {
  function isAdminRoute(pathname: string): boolean {
    return /^\/admin(\/.*)?$/.test(pathname) || /^\/fr\/admin(\/.*)?$/.test(pathname);
  }
  ...
  const nonAdminPaths = ['/dashboard', '/profile', '/fr/dashboard', '/api/admin'];
...
describe('API routes return 401 for unauthenticated requests (Req 2.5)', () => {
  ...
  it('protected API route without userId returns 401', () => {
    expect(handleApiRoute('/api/auth/permissions', null, false)).toEqual({ status: 401 });
  });

  it('protected API route with userId passes through', () => {
    expect(handleApiRoute('/api/auth/permissions', 'user_123', false)).toBe('next');
  });
...
describe('Authenticated users can access protected routes (Req 2.3)', () => {
  ...
  it('authenticated user can access /dashboard', () => {
    expect(shouldAllowAccess('/dashboard', 'user_123', false, false, false)).toBe(true);
  });

  it('unauthenticated user cannot access /dashboard', () => {
    expect(shouldAllowAccess('/dashboard', null, false, false, false)).toBe(false);
  });

  it('authenticated admin can access /admin/users', () => {
    expect(shouldAllowAccess('/admin/users', 'user_123', false, true, true)).toBe(true);
  });

  it('authenticated non-admin cannot access /admin/users', () => {
    expect(shouldAllowAccess('/admin/users', 'user_123', false, true, false)).toBe(false);
  });
