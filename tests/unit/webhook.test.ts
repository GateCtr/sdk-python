...
describe('Clerk Webhook Handler Unit Tests', () => {
  ...
  describe('user.created event', () => {
    ...
    it('skips creation when user already exists (idempotency)', async () => {
      ...
      const existingUser = { id: 'db-user-4', clerkId: 'user_created_t4', email: 'test@example.com' };
  ...
  describe('user.updated event', () => {
    ...
    it('updates the User record with new email, name, and avatarUrl', async () => {
      ...
      const existingUser = { id: 'db-user-u1', clerkId: 'user_updated_t1', email: 'old@example.com', name: 'Old Name', avatarUrl: null };
      const updatedUser = { ...existingUser, email: 'updated@example.com', name: 'Updated Name', avatarUrl: 'https://img.clerk.com/new-avatar.jpg' };
      ...
      expect(prisma.user.update).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          data: expect.objectContaining({ email: 'updated@example.com', name: 'Updated Name' }),
        }),
      );
    });
    ...
    it('logs an audit entry with action user.updated', async () => {
      ...
      const existingUser = { id: 'db-user-u2', clerkId: 'user_updated_t2', email: 'old@example.com', name: 'Old', avatarUrl: null };
      const updatedUser = { ...existingUser, email: 'updated@example.com', name: 'Updated Name' };
      ...
      expect(logAudit).toHaveBeenCalledWith(
        expect.objectContaining({ resource: 'user', action: 'user.updated', success: true }),
      );
    });
  ...
  describe('user.deleted event', () => {
    ...
    it('sets isActive to false (soft delete) instead of removing the record', async () => {
      ...
      const existingUser = { id: 'db-user-d1', clerkId: 'user_deleted_t1', email: 'test@example.com', isActive: true };
    ...
    it('logs an audit entry with action user.deleted', async () => {
      ...
      const existingUser = { id: 'db-user-d2', clerkId: 'user_deleted_t2', email: 'test@example.com', isActive: true };
      prisma.user.findUnique.mockResolvedValue(existingUser);
      prisma.user.update.mockResolvedValue({ ...existingUser, isActive: false });
      ...
      expect(logAudit).toHaveBeenCalledWith(
        expect.objectContaining({ resource: 'user', action: 'user.deleted', success: true }),
      );
    });
  ...
  describe('Error handling', () => {
    ...
    it('returns 500 when database throws during user.updated', async () => {
      ...
      prisma.user.findUnique.mockResolvedValue({ id: 'db-user-e2', clerkId: 'user_err_t2', email: 'old@example.com', name: 'Old', avatarUrl: null });
    ...
    it('returns 500 when database throws during user.deleted', async () => {
      ...
      prisma.user.findUnique.mockResolvedValue({ id: 'db-user-e3', clerkId: 'user_err_t3', isActive: true });
      prisma.user.update.mockRejectedValue(new Error('DB write failed'));
    ...
    it('returns 200 for unhandled event types without errors', async () => {
      const unknownPayload = { type: 'session.created', data: { id: 'sess_unique_001' } };
    ...
    it('returns 500 when CLERK_WEBHOOK_SECRET is not configured', async () => {
      ...
      const { POST } = await import('@/app/api/webhooks/clerk/route');
      const req = makeRequest(makeCreatedPayload('user_no_secret'), { 'svix-id': 'evt_no_secret_001' });
