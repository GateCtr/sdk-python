...
describe('GET /api/auth/permissions', () => {
  ...
  beforeEach(async () => {
    ...
    mockFindUnique = prisma.user.findUnique as unknown as ReturnType<typeof vi.fn>;

    const permsModule = await import('@/lib/permissions');
    mockGetUserPermissions = permsModule.getUserPermissions as unknown as ReturnType<typeof vi.fn>;
...
describe('POST /api/auth/permissions/check', () => {
  ...
  beforeEach(async () => {
    ...
    const permsModule = await import('@/lib/permissions');
    mockHasPermission = permsModule.hasPermission as unknown as ReturnType<typeof vi.fn>;
  ...
  it('resolves the internal user ID from the Clerk ID before checking permission', async () => {
    ...
    expect(mockHasPermission).toHaveBeenCalledWith('internal_456', 'audit:read');
  });
  ...
  it('correctly checks analytics:read permission', async () => {
    ...
    expect(res.status).toBe(200);
    expect(body).toEqual({ allowed: true });
    expect(mockHasPermission).toHaveBeenCalledWith('user_123', 'analytics:read');
  });
