...
function mockRolesResult(data: ReturnType<typeof useRoles>['data'], isLoading: boolean) {
...
describe('RoleGate', () => {
  ...
  it('requireAll=false renders fallback when user has none of the roles', () => {
    ...
    expect(screen.queryByText('Partial access content')).not.toBeInTheDocument();
    expect(screen.getByText('No access')).toBeInTheDocument();
  });

  it('requireAll=true renders children only when user has ALL of the roles', () => {
    mockUseRoles.mockReturnValue(mockRolesResult(['ADMIN', 'SUPER_ADMIN'], false));
  ...
  it('requireAll=true renders fallback when user has only some of the roles', () => {
    ...
    render(
      <RoleGate roles={['ADMIN', 'SUPER_ADMIN']} requireAll fallback={<span>Insufficient roles</span>}>
        <span>Super admin content</span>
      </RoleGate>
    );
