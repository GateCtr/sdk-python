...
function makeAuditResult(logs: ReturnType<typeof makeLog>[], total = logs.length) {
  return {
    logs,
    total,
    page: 1,
...
describe('AuditLogsPage', () => {
  ...
  describe('access control', () => {
    ...
    it('redirects when requirePermission throws', async () => {
      mockRequirePermission.mockRejectedValue(new Error("Unauthorized: Missing required permission 'audit:read'"));
  ...
  describe('pagination', () => {
    ...
    it('passes the page number from searchParams', async () => {
      const page = await AuditLogsPage({ searchParams: Promise.resolve({ page: '3' }) });
      render(page);
    ...
    it('clamps invalid page values to 1', async () => {
      const page = await AuditLogsPage({ searchParams: Promise.resolve({ page: '-5' }) });
      render(page);
