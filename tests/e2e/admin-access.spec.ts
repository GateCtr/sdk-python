...
test.describe('Admin routes require authentication (Requirements 6.1, 6.2, 6.3)', () => {
  ...
  for (const route of adminRoutes) {
    test(`unauthenticated user is redirected from ${route} to sign-in`, async ({ page }) => {
  ...
  test('redirect from /admin/users does not expose a 500 error', async ({ page }) => {
    await page.goto('/admin/users');
  ...
  test('redirect from /admin/audit-logs does not expose a 500 error', async ({ page }) => {
...
test.describe('French locale admin routes require authentication (Requirement 6.1)', () => {
  ...
  test('French admin redirect preserves locale in sign-in URL', async ({ page }) => {
    /**
...
test.describe('Admin route redirect behavior (Requirement 6.3)', () => {
  test('redirect from /admin/users lands on a sign-in page (not 404 or 500)', async ({ page }) => {
    await page.goto('/admin/users');
  ...
  test('redirect from /admin/audit-logs lands on a sign-in page', async ({ page }) => {
    await page.goto('/admin/audit-logs');
    await page.waitForURL(/sign-in/, { timeout: 10000 });
    expect(page.url()).toContain('sign-in');
  });

  test('redirect from /admin/waitlist lands on a sign-in page', async ({ page }) => {
    await page.goto('/admin/waitlist');
    await page.waitForURL(/sign-in/, { timeout: 10000 });
    expect(page.url()).toContain('sign-in');
  });

  test('wildcard /admin/* routes redirect unauthenticated users to sign-in', async ({ page }) => {
    /**
     * Requirement 6.1: Admin_Area SHALL be accessible at routes /admin/* and /fr/admin/*
     * (meaning all sub-routes are protected)
     */
    const wildcardRoutes = ['/admin/users', '/admin/audit-logs', '/admin/waitlist'];
...
test.describe('Admin sidebar structure (Requirement 6.4)', () => {
  test('sign-in page (reached via admin redirect) renders without errors', async ({ page }) => {
    /**
...
test.describe('Admin area access control summary (Requirements 6.2, 6.3)', () => {
  test('all English admin routes redirect unauthenticated users', async ({ page }) => {
  ...
  test('all French admin routes redirect unauthenticated users', async ({ page }) => {
    const routes = ['/fr/admin/users', '/fr/admin/audit-logs', '/fr/admin/waitlist'];
    for (const route of routes) {
      await page.goto(route);
      await page.waitForURL(/sign-in/, { timeout: 10000 });
      expect(page.url()).toContain('sign-in');
  ...
  test('sign-in page is reachable after admin redirect (not a redirect loop)', async ({ page }) => {
    await page.goto('/admin/users');
    await page.waitForURL(/sign-in/, { timeout: 10000 });
