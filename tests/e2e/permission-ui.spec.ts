...
const ADMIN_SIDEBAR_ITEMS = ['Users', 'Plans', 'Feature Flags', 'Audit Logs', 'System', 'Waitlist'];
...
test.describe('Permission-based access control: unauthenticated redirects (Requirements 5.1, 5.2, 6.2)', () => {
   ...
   */
  test('unauthenticated user is redirected from /admin/users to sign-in', async ({ page }) => {
    await page.goto('/admin/users');
    await page.waitForURL(/sign-in/, { timeout: 10000 });
    expect(page.url()).toContain('sign-in');
  });

  test('unauthenticated user is redirected from /admin/audit-logs to sign-in', async ({ page }) => {
    await page.goto('/admin/audit-logs');
    await page.waitForURL(/sign-in/, { timeout: 10000 });
    expect(page.url()).toContain('sign-in');
  });

  test('unauthenticated user is redirected from /admin/waitlist to sign-in', async ({ page }) => {
...
test.describe('AdminSidebar not visible to unauthenticated users (Requirement 6.4)', () => {
  ...
  test('sign-in page (reached via redirect from /admin/users) does not show admin sidebar', async ({
    ...
    for (const item of ADMIN_SIDEBAR_ITEMS) {
      await expect(page.locator('nav').filter({ hasText: item })).toHaveCount(0);
    }
  });

  test('sign-in page does not contain admin navigation links', async ({ page }) => {
    ...
    await expect(page.locator('a[href*="/admin/feature-flags"]')).toHaveCount(0);
...
test.describe('Public pages do not show admin-only content (Requirements 5.1, 5.2)', () => {
   ...
   */
  test('home page (/) does not contain admin sidebar items', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
  ...
  test('waitlist page (/waitlist) does not contain admin sidebar items', async ({ page }) => {
  ...
  test('waitlist page does not show admin-only navigation', async ({ page }) => {
    await page.goto('/waitlist');
    await page.waitForLoadState('networkidle');
...
test.describe('Permission API returns 401 for unauthenticated requests (Requirements 5.1, 5.2)', () => {
   ...
   */
  test('GET /api/auth/permissions returns 401 for unauthenticated request', async ({ page }) => {
...
test.describe('RoleGate: role-based access enforced via redirect (Requirement 6.2)', () => {
   ...
   */
  test('admin area requires authentication (no roles = no access)', async ({ page }) => {
  ...
  test('French admin area also enforces role-based access', async ({ page }) => {
    await page.goto('/fr/admin/users');
    await page.waitForURL(/sign-in/, { timeout: 10000 });
    expect(page.url()).toContain('sign-in');
  });

  test('French admin audit-logs enforces role-based access', async ({ page }) => {
...
test.describe('PermissionGate: permission-gated content not leaked (Requirements 5.1, 5.2)', () => {
   ...
   */
  test('sign-in page does not contain permission-gated admin content', async ({ page }) => {
    await page.goto('/sign-in');
    await page.waitForLoadState('networkidle');
  ...
  test('public home page does not expose permission-gated content', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
