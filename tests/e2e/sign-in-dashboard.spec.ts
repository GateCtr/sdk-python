...
test.describe('Unauthenticated redirect to sign-in (Requirement 2.3)', () => {
  test('visiting /dashboard redirects unauthenticated user to sign-in', async ({ page }) => {
    await page.goto('/dashboard');
    await page.waitForURL(/sign-in/, { timeout: 10000 });
    expect(page.url()).toContain('sign-in');
  });

  test('redirect from /dashboard includes redirect_url parameter', async ({ page }) => {
    await page.goto('/dashboard');
    await page.waitForURL(/sign-in/, { timeout: 10000 });
    const url = page.url();
  ...
  test('visiting /onboarding redirects unauthenticated user to sign-in', async ({ page }) => {
    await page.goto('/onboarding');
    await page.waitForURL(/sign-in/, { timeout: 10000 });
    expect(page.url()).toContain('sign-in');
  });

  test('visiting /admin/users redirects unauthenticated user to sign-in', async ({ page }) => {
    await page.goto('/admin/users');
    await page.waitForURL(/sign-in/, { timeout: 10000 });
    expect(page.url()).toContain('sign-in');
  });

  test('visiting /admin/audit-logs redirects unauthenticated user to sign-in', async ({ page }) => {
...
test.describe('Sign-in page structure (Requirement 1.1)', () => {
  ...
  test('sign-in page renders Clerk sign-in component', async ({ page }) => {
    ...
    const clerkElement = page
      .locator('[data-clerk-component], .cl-rootBox, .cl-signIn-root, form, iframe')
      .first();
...
test.describe('Protected page navigation for unauthenticated users (Requirement 2.3)', () => {
  const protectedRoutes = ['/dashboard', '/onboarding', '/admin/users', '/admin/audit-logs'];
...
test.describe('Sign-out behavior (Requirement 1.7)', () => {
  test('sign-out route exists and redirects unauthenticated user', async ({ page }) => {
  ...
  test('after sign-out, accessing /dashboard requires re-authentication', async ({ page }) => {
...
test.describe('French locale protected route redirects (Requirement 2.3)', () => {
  ...
  test('visiting /fr/onboarding redirects unauthenticated user to sign-in', async ({ page }) => {
